...
describe("toInt", function () {
  ...
  it.each([["13", 13], ["3.8", 3], ["98", 98]])("val %p", function(input, output) {
    expect(convert.toInt(input)).toBe(output);
  });
});
...
describe("toFloat", function () {
  ...
  it.each([["13", 13], ["3.8", 3.8], ["98.673333", 98.673333]])("val %p", function(input, output) {
    expect(convert.toFloat(input)).toBe(output);
  });
});
